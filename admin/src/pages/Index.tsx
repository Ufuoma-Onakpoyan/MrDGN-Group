...
const Index = () => {
  ...
  const loadData = async () => {
    try {
      ...
      const props = propertiesData.slice(0, 6);
      const testis = testimonialsData.slice(0, 3);

      setProperties(props);
      setTestimonials(testis);
      setStats({
        totalProperties: propertiesData.length,
        soldProperties: propertiesData.filter((p: { status: string }) => p.status === 'sold').length,
        happyClients: testimonialsData.length,
      });
    } catch (error) {
